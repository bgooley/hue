...
class HiveServerTable(Table):
  ...
  def path_location(self):
    try:
      ...
      col_row_index = describe.index('Location:')
      return describe[col_row_index]['data_type']
    except:
      return None
  ...
  def cols(self):
    ...
    try:
      col_row_index = 2
      end_cols_index = map(itemgetter('col_name'), cols[col_row_index:]).index('')
      return cols[col_row_index:][:end_cols_index] + self._get_partition_column()
    except:
      return cols
  ...
  @property
  def properties(self):
    return self.describe # All but the columns for now
